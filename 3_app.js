...
function getrestaurant(restaurants){
    ...
    restaurants.forEach(restaurant => {
        ...
        card.classList.add('card');                     // we can also use- card.classname('card')

        // Create div for image and off
        ...
        off.innerHTML = `Flat ${restaurant.offers}% Off`;
        off.classList.add("off");
        images.appendChild(off);
...
document.getElementById("Alcohol").addEventListener('click',()=>{
    ...
    document.getElementById('root').replaceChildren();      // to remove the children of root div
...
document.getElementById("Offers").addEventListener('click',()=>{
    const result = restaurants.filter((obj)=>obj.offers>0);
    document.getElementById('root').replaceChildren();
    getrestaurant(result);
})
...
document.getElementById('applyFilter').addEventListener('click',()=>{
    ...
    if(answer==="rating"){
        restaurants.sort((a,b)=>b.rating-a.rating);         // descending order- (b-a), ascending order- (a-b)
    }
    ...
    restaurants.sort((a, b) => b.offers - a.offers);
    }
    document.getElementById('root').replaceChildren();
